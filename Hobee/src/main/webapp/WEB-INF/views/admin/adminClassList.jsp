		...
		<div class="table-container">
			...
			<div class="pagination">
			    ...
			    <c:if test="${pageVO.currentPage > 1}">
			        <a href="${pageContext.request.contextPath}/admin/adminClassList?pageNum=1&filter=${filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[처음]</a>
			    </c:if>
			    
			    <!-- 10 페이지 이전 -->
			    <c:if test="${pageVO.startPage > pageVO.pageBlock}">
			        <a href="${pageContext.request.contextPath}/admin/adminClassList?pageNum=${pageVO.startPage - pageVO.pageBlock}&filter=${filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[이전]</a>
			    </c:if>
			    ...
			    <c:forEach var="i" begin="${pageVO.startPage}" end="${pageVO.endPage}" step="1">
			        <c:choose>
			            <c:when test="${i == pageVO.currentPage}">
			                <a href="${pageContext.request.contextPath}/admin/adminClassList?pageNum=${i}&filter=${filter}&search=${pageVO.search}&searchList=${pageVO.searchList}" class="active">${i}</a>
			            </c:when>
			            <c:otherwise>
			                <a href="${pageContext.request.contextPath}/admin/adminClassList?pageNum=${i}&filter=${filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">${i}</a>
			            </c:otherwise>
			        </c:choose>
			    </c:forEach>
			    ...
			    <c:if test="${pageVO.endPage < pageVO.pageCount}">
			        <a href="${pageContext.request.contextPath}/admin/adminClassList?pageNum=${pageVO.startPage + pageVO.pageBlock}&filter=${filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[다음]</a>
			    </c:if>
			    
			    <!-- 맨 끝으로 -->
			    <c:if test="${pageVO.currentPage < pageVO.pageCount}">
			        <a href="${pageContext.request.contextPath}/admin/adminClassList?pageNum=${pageVO.pageCount}&filter=${filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[끝]</a>
			    </c:if>
			</div>
		</div>
	</main>
