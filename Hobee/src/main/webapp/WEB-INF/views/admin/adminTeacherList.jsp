...
<!-- 		</div> -->
		...
		<div class="table-container">
			<div class="search-wrapper">
				<form action="${ pageContext.request.contextPath }/admin/adminTeacherList" class="search-form" id="search_form">
				...
				<a href="${ pageContext.request.contextPath }/admin/adminTeacherList" class="search-btn" id="search_btn">전체 강의</a>
			</form>
			</div>
			<table>
				<thead>
			...
			<div class="pagination">
			    ...
			    <c:if test="${pageVO.currentPage > 1}">
			        <a href="${pageContext.request.contextPath}/admin/adminTeacherList?pageNum=1&filter=${param.filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[처음]</a>
			    </c:if>
			    
			    <!-- 10 페이지 이전 -->
			    <c:if test="${pageVO.startPage > pageVO.pageBlock}">
			        <a href="${pageContext.request.contextPath}/admin/adminTeacherList?pageNum=${pageVO.startPage - pageVO.pageBlock}&filter=${param.filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[이전]</a>
			    </c:if>
			    ...
			    <c:forEach var="i" begin="${pageVO.startPage}" end="${pageVO.endPage}" step="1">
			        <c:choose>
			            <c:when test="${i == pageVO.currentPage}">
			                <a href="${pageContext.request.contextPath}/admin/adminTeacherList?pageNum=${i}&filter=${param.filter}&search=${pageVO.search}&searchList=${pageVO.searchList}" class="active">${i}</a>
			            </c:when>
			            <c:otherwise>
			                <a href="${pageContext.request.contextPath}/admin/adminTeacherList?pageNum=${i}&filter=${param.filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">${i}</a>
			            </c:otherwise>
			        </c:choose>
			    </c:forEach>
			    ...
			    <c:if test="${pageVO.endPage < pageVO.pageCount}">
			        <a href="${pageContext.request.contextPath}/admin/adminTeacherList?pageNum=${pageVO.startPage + pageVO.pageBlock}&filter=${param.filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[다음]</a>
			    </c:if>
			    
			    <!-- 맨 끝으로 -->
			    <c:if test="${pageVO.currentPage < pageVO.pageCount}">
			        <a href="${pageContext.request.contextPath}/admin/adminTeacherList?pageNum=${pageVO.pageCount}&filter=${param.filter}&search=${pageVO.search}&searchList=${pageVO.searchList}">[끝]</a>
			    </c:if>
			</div>
